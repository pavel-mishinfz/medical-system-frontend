...
const ProfileImg = ({userData, setUserData, userId}) => {
  ...
  const updateAvatar = async (imgSrc) => {
    ...
    try {
      const response = await axios.patch('http://' + window.location.hostname + `:5000/users/${userId}/image`, formData, {
        headers: {
  ...
  const deleteAvatar = async () => {
    try {
      const response = await axios.delete('http://' + window.location.hostname + `:5000/users/${userId}/image`, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem('authToken')}`,
          ContentType: 'multipart/form-data'
        }
  ...
  return (
    ...
    <div className="profile__img">
        <img src={`http://${window.location.hostname}:5000/${userData.img ? userData.img : 'storage/img_user_none.jpg'}`} alt="user avatar" />
        {userData.img && (
